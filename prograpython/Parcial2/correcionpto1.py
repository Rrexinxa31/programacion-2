...
def agregar_producto(JDRC_inventario):
    while True:  # Bucle para validar el tipo de producto
        JDRC_tipo = input("Ingrese el tipo de producto (Electronico/Alimento): ").strip().lower()
        
        if JDRC_tipo in ["electronico", "alimento"]:
            break  # Sale del bucle si el tipo es válido
        else:
            print("Tipo de producto no válido. Por favor, ingrese 'Electronico' o 'Alimento'.")
    ...
    if JDRC_tipo == "electronico":
        ...
        while True:  # Validar la fecha de vencimiento
            JDRC_fecha_vencimiento = input("Fecha de vencimiento (YYYY-MM-DD): ")
            try:
                # Validar que la fecha sea correcta
                JDRC_fecha_vencimiento = datetime.strptime(JDRC_fecha_vencimiento, "%Y-%m-%d").date()
...
def main():
    ...
    JDRC_producto1 = Electronico("Smartphone", 500, 10, 12)  # Producto electronico
    JDRC_producto2 = Alimento("Arroz", 1.5, 50, "2025-12-31")  # Producto de alimento
    
    # Agregar los productos al inventario
    JDRC_inventario.agregar_producto(JDRC_producto1)
    JDRC_inventario.agregar_producto(JDRC_producto2)
    ...
    while True:
        ...
        if JDRC_opcion == "1":
            ...
            JDRC_inventario.guardar_csv("inventario1.csv")
